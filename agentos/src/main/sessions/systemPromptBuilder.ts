  ...
  if (slackCtx && useHeadless) {
    ...
    if (slackCtx.threadTs && agentRole === 'task-main') {
      ...
      slackPrompt =
        ...
        `- upload_file(channel_id, thread_ts, file_path, filename?, initial_comment?): upload a file to the Slack thread (file_path must be absolute, under /workspace or /tmp).\n` +
