...
function rowToThread(row: ThreadRow): StoredThread {
  return {
    ...
    currentReaction: (row.currentReaction as Thread['currentReaction']) ?? undefined,
    unreadCount: row.unreadCount ?? 0,
    unreadKind: (row.unreadKind as Thread['unreadKind']) ?? undefined,
...
function threadToInsert(t: StoredThread): ThreadInsert {
  return {
    ...
    currentReaction: t.currentReaction ?? null,
    unreadCount: t.unreadCount ?? 0,
    unreadKind: t.unreadKind ?? null,
