...
- **Autopilot mode** — a secondary Claude planner decides whether to send the next user-behalf message after each assistant turn, enabling multi-turn autonomous task completion with a configurable maximum consecutive-turn limit (default 10). Autopilot is enabled at thread start when `agents.autopilot.enabled` is true in effective settings; visual indicators include a pulsing badge in the thread list, a robot icon below autopilot-posted messages, and an inline reasoning bubble showing the last decision. Stalled turns auto-recover. Toggle is available in the composer for all providers. The **planner provider and model** are configurable independently of the thread's own provider in global Settings → Autopilot; projects can override turn limits and transcript length.
- **Slack integration** — inbound Slack messages trigger thread creation or route into existing thread bindings. The in-app Thread view is the source of truth; Slack is a bound echo medium. Agents use `agentos-thread` MCP tools (`post_update`, `ask_clarification`, `upload_file`) so updates are saved in the Thread view and mirrored to Slack when connected. Supports file attachments; agent output is converted from Markdown to Slack's mrkdwn format automatically. Coding tasks require explicit `ask_clarification` approval before proceeding. Catches up missed messages on system wake. Optional **requireMention** toggle (`SlackSettings.requireMention`) drops new root-thread messages that do not contain an @mention of the bot; replies in existing threads always pass through.
...
- **Meeting recording, continuous capture & AI notes** — record meetings directly in the app, transcribe locally on-device via fire-and-forget background processing, and auto-generate structured notes via the bundled `meeting-notes` skill which also auto-titles the thread. Continuous capture stores rolling 5-minute segments that can be summarized by selecting a time window. Recordings and segments are persisted to a `recordings` table in the threads DB and are accessible to agents via the `agentos-recordings` MCP server (dynamic port). A bottom-left recording pill persists across tab switches to track the active recording thread. `FEATURES.MEETINGS`, `FEATURES.VOICE_FLOW`, and `FEATURES.KANBAN` currently default to `true` in `src/shared/features.ts`.
...
|---|---|---|---|
| Desktop shell | Electron | 41.0.2 | Main + renderer + preload processes |
...
| Memory DB | better-sqlite3 + sqlite-vec | 12.8 / 0.1.9 | One SQLite file per project in `~/.agentos/memory/projects/` |
...
| **agentos-recordings MCP** | An MCP server (dynamic port, `src/main/integrations/recordingsMcpServer.ts`) that exposes `get_recording_meta`, `get_transcript`, `list_recordings`, `list_segments`, and `get_window_transcript` so agents can access persisted meeting recordings and continuous-capture windows. |
| **Meeting recording** | A feature that captures and transcribes meeting audio and generates structured notes via the bundled `meeting-notes` skill. Processing is fire-and-forget (background transcription + thread creation); status shown in the recording tab. Recordings and continuous-capture segments are stored in the threads SQLite DB (`recordings` table). Gated by `FEATURES.MEETINGS`, currently enabled. |
