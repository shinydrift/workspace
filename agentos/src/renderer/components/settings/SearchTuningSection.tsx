...
type MemoryPresetKey = keyof Pick<
  ...
  'maxResults' | 'minScore' | 'vectorWeight' | 'textWeight' | 'halfLifeDays' | 'mmrLambda'
>;
...
const PRESETS: Preset<MemorySearchSettings, MemoryPresetKey>[] = [
  {
    name: 'default',
    label: 'Default',
  ...
  },
  {
    ...
    settings: { maxResults: 5, minScore: 0.3, vectorWeight: 0.8, textWeight: 0.2, halfLifeDays: 30, mmrLambda: 0.85 },
  },
  {
    name: 'broad',
    label: 'Broad',
    description: 'More results, lower threshold — good for exploratory recall.',
    settings: { maxResults: 15, minScore: 0.05, vectorWeight: 0.6, textWeight: 0.4, halfLifeDays: 90, mmrLambda: 0.5 },
  },
  {
    name: 'custom',
    label: 'Custom',
...
type SliderKey = keyof Pick<
  MemorySearchSettings,
  'maxResults' | 'minScore' | 'vectorWeight' | 'textWeight' | 'halfLifeDays' | 'mmrLambda' | 'sessionRetentionDays'
>;
...
const SLIDER_CONFIG: {
  ...
  { key: 'halfLifeDays', label: 'Decay half-life', low: '1d', high: '365d', min: 1, max: 365, step: 1, defaultVal: 45 },
...
export function SearchTuningSection() {
  ...
  const patchSearch = (patch: Partial<MemorySearchSettings>) =>
    memory.setMemorySearch({ ...memory.memorySearch, ...patch });

  const activePreset = useMemo(
    () => (forceCustom ? 'custom' : detectPreset(PRESETS, memory.memorySearch, PRESET_KEYS)),
    [forceCustom, memory.memorySearch]
  );

  const applyPreset = (preset: Preset<MemorySearchSettings, MemoryPresetKey>) => {
